# -*- coding: utf-8 -*-
"""CODE2 Forest Fire.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/15Pn6TChrUgGZFiSXaRamSwFmIWwfI6XC
"""
...
def evaluate_one(Alg, params, env, n, period_size=1):
  ...
  ce = np.zeros(n // period_size)
  past_pulls = np.zeros((n, env.K))
  past_pulls[0, :] = 1
  Lambda = 1e-3 * np.eye(env.d)
  ...
  for t in range(n):
    ...
    # track cross-entropy between action distribution and variance distribution
    if t > 0:
      Sigma = np.linalg.inv(Lambda)
      score = np.log(1 + (env.X.dot(Sigma) * env.X).sum(axis=1))
      feasible_arms = past_pulls[max(t - 100, 0) : t, :].sum(axis=0) > 0
      ce[t // period_size] -= np.log(score[arm] / score[feasible_arms].sum())
    Lambda += np.outer(env.X[arm, :], env.X[arm, :])
    past_pulls[t, arm] = 1
...
def evaluate(Alg, params, env, n=1000, period_size=1, printout=True):
  ...
  num_exps = len(env)
  regret = np.zeros((n // period_size, num_exps))
  ce = np.zeros((n // period_size, num_exps))
...
class CODE(LinBanditAlg):
  def __init__(self, env, n, params):
    self.acquisition = "policy"
    self.acquisition = "action" # Results in deterministic policy
  ...
  def get_arm(self, t):
    ...
    if self.acquisition == "action":
      # minimum variance action
      V = self.Lambda[np.newaxis, :, :] + np.einsum("ij,ik->ijk", self.env.X, self.env.X)
      _, logdetV = np.linalg.slogdet(np.linalg.inv(V))
      best = np.argmin(logdetV[self.active_arms])
      arm = self.active_arms[best]
    elif self.acquisition == "policy":
      if self.active_arms.size > 1:
        # minimum D-optimal design policy
...
D = df.values.tolist()
# D = []
# with open('heart_failure_clinical_records_dataset.csv', newline='') as csvfile:
...
print(np.shape(D), D[1])
# D = D[1:, : 3].astype(int)
num_wines, features = np.shape(D)
features = features - 1

M = np.zeros((num_wines, features))
M[:, :features] = D[:, :features]
W = np.zeros((num_wines,1))
W[:, 0] = D[:,features]

ndx = np.random.permutation(num_wines)
M = M[ndx, :]
W = W[ndx, :]

# print(M, W)
# print(np.shape(M), np.shape(W))
...
np.random.seed(6) # fix a seed for reproducibility
d = 12  # number of features
K = 200  # number of arms
sigma = 0.8  # reward noise
n = 10000  # horizon
...
for run in range(num_runs):
  ...
  wine = np.random.randint(num_wines, size=K)
  type_ = np.random.randint(Theta.shape[0])
  # wine = np.random.randint(Theta.shape[0])
  envs.append(LinBandit(X[:K, :d], Theta[type_, :d], sigma=sigma))
...
#   alg[1]["sigma"] = sigma
#   regret, _ = evaluate(alg_class, alg[1], envs, n)
...
# plt.tight_layout()
# plt.savefig("Figures/MovieLens.pdf", format="pdf", dpi=1200, bbox_inches=0)
# plt.show()
...
# plt.tight_layout()
# plt.savefig("Figures/MovieLensTuning.pdf", format="pdf", dpi=1200, bbox_inches=0)
# plt.show()