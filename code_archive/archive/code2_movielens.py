# -*- coding: utf-8 -*-
"""CODE2 MovieLens.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1qF2CfRNZo9lK0LNuajhDPYWABgxMQG9B
"""
...
def evaluate_one(Alg, params, env, n, period_size=1):
  ...
  for t in range(n):
    ...
    if t > 0:
      ...
      # print(score)
      # try:
      feasible_arms = past_pulls[max(t - 50, 0) : t, :].sum(axis=0) > 0
      # ce[t // period_size] -= np.log(score[arm] / score[feasible_arms].sum())
      ce[t // period_size] -= np.log( max(score[arm], 0.00001) / score[feasible_arms].sum())
      # except RuntimeWarning:
      #   ipdb.set_trace()
      #   ce[t // period_size] = 1.0
...
sigma = 0.811  # reward noise
n = 10000 # horizon
num_runs = 100  # number of random runs
...
for run in range(num_runs):
  ...
  movies = np.random.randint(num_movies, size=K)
  user = np.random.randint(Theta.shape[0])
  envs.append(LinBandit(X[movies, :], Theta[user, :], sigma=sigma))
