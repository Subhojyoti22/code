...
# # load data
# with open('data_linear.pickle', 'rb') as f:
...
regret_algs, ce_algs = data[0], data[1]
...
for alg in algs:
  ...
  regret, ce = regret_algs[:, :, algs.index(alg)], ce_algs[:, :, algs.index(alg)]
  ...
  plt.plot(step, cum_ce.mean(axis=1),
...
plt.legend(loc="lower right", ncol=2, frameon=False, fontsize=9)
plt.yscale('log')
# plt.ylim(1, 200000) # required in wine
...
plt.xlabel("Round n", fontsize=9)
plt.ylabel("Uncertainty loss", fontsize=9)
plt.ylim(0.1, 100)
plt.yscale('log')

plt.tight_layout()

# plt.savefig("Figures/Linear2.pdf", format="pdf", dpi=1200, bbox_inches=0)
# plt.savefig("Figures/Linear2.png", format="png", dpi=1200, bbox_inches=0)
